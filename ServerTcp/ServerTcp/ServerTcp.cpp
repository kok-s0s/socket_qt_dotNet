	...
	: QMainWindow(parent)
{
	...
	QString fireHvcA[] = { "013", "048", "081", "114", "141", "163", "178", "196", "214", "227", "242", "255" };
	QString focusDepthA[] = { "020", "040", "060", "080", "110", "140", "180", "230" };
	m_testTypesCount = 0;
	QString temp = "";
	...
	for (int i = 0; i < 3; ++i) {
		for (auto hvca : fireHvcA) {
			for (auto deptha : focusDepthA) {
				temp = fireFreqB0A[i] + "_" + fireFreqB1A[i] + "_" + hvca + "_" + deptha;
	...
	connect(m_pTcpServer, &QTcpServer::newConnection, this, &ServerTcp::server_New_Connect);
}
...
void ServerTcp::on_btnListen_clicked()
{
	...
	{
		if (!m_pTcpServer->listen(QHostAddress::Any, ui.lineEditPort->text().toInt()))
		{
...
void ServerTcp::server_New_Connect()
{
	...
	connect(m_pTcpSocket, &QTcpSocket::readyRead, this, &ServerTcp::socket_Read_Data);
	connect(m_pTcpSocket, &QTcpSocket::disconnected, this, &ServerTcp::socket_Disconnected);
	...
	if (!buffer.isEmpty())
	{
		...
		if (str == "connect") {
			...
			if (str.toInt() < m_testTypesCount) {
				QStringList paramlist = m_testTypesTable.value(str.toInt()).split(QRegExp("_"));
				qDebug() << paramlist[0].toInt() << " " << paramlist[1].toInt() << " " << paramlist[2].toInt() << " " << paramlist[3].toDouble() << endl;
				//ocean::GetOceanInterface()->setAIMSTestParam(paramlist[0].toInt(), paramlist[1].toInt(), paramlist[2].toInt(), paramlist[3].toDouble());
...
void ServerTcp::socket_Disconnected()
{
	qDebug() << "Disconnected!";
}
